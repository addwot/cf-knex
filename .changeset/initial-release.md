...
Connect by URL, by a Hyperdrive binding, by a D1 binding, or by explicit credentials; `fromEnv()` picks whichever is present. Failures arrive as `CfKnexError` with a closed `code` union of ten values, and connection URLs are redacted before they reach driver-inference error messages.
